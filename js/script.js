...
.then(json => {
    ...
    prices.forEach(el => {
        ...
        $template.querySelector(".titulomaterial").innerHTML = `${productData[0].name.toUpperCase()}`;
        $template.querySelector(".arepa").setAttribute("data-price", el.id);
        $template.querySelector("img").src = productData[0].images[0];
        $template.querySelector("img").alt = productData[0].name;
        $template.querySelector("figcaption").innerHTML = `${FormatoDeMoneda(el.unit_amount_decimal).toUpperCase()}` + '/MES';
